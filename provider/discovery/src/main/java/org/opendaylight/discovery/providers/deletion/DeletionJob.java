...
//import com.ciena.opendaylight.discovery.samples.device.DeviceProvider;
...
public class DeletionJob implements Runnable {
    ...
    private static final Logger log = LoggerFactory
            .getLogger(DeletionJob.class);
    private final DeleteNetworkElement notification;
    ...
    public DeletionJob(RpcProviderRegistry context,
            NotificationProviderService notificationService,
            DeleteNetworkElement notification) {
     ...
     */
    public final DeleteNetworkElement getNotification() {
    ...
    public void run() {
        try {
            log.debug("JOB : Delete: destroy : RUN : {}, {}, {}",
                    this.notification.getRequestId(),
                    this.notification.getNetworkElementIp(),
                    this.notification.getNodeId());
            // Invoke the routed RPC and wait for the result.
            DiscoveryDeletionService service = this.rpcRegistry
                    .getRpcService(DiscoveryDeletionService.class); // returns
            // the proxy
            ...
            input.setNetworkElementType(this.notification
            ...
            InstanceIdentifier<NetworkElementType> id = InstanceIdentifier
                    .builder(NetworkElementTypes.class)
                    .child(NetworkElementType.class,
                            new NetworkElementTypeKey(notification
                                    .getNetworkElementType())).build();
            input.setNetworkElementTypeRef(id);

            log.debug(
                    "ROUTED_RPC : Delete: destroyNetworkElement : INVOKE : {}, {}, {}",
                    input.getRequestId(), input.getNetworkElementIp(),
                    input.getNodeId());

            Future<RpcResult<DestroyNetworkElementOutput>> future = service
                    .destroyNetworkElement(input.build());

            RpcResult<DestroyNetworkElementOutput> rpcresult = future.get();// Publish appropriate notification based on the result of the  deletion
            if (rpcresult.isSuccessful()) {
                ...
                if (Success.class.isInstance(result)) {
                     ...
                     */
                    final NetworkElementDeleted event = new NetworkElementDeletedBuilder()
                    .setChanges(true)
                    ...
                    .setNetworkElementIp(
                            ...
                            .setNetworkElementType(
                                    ...
                                    .build();
                    log.debug(
                            "EVENT : Delete: NetworkElementDeleted : PUBLISH : {}, {}, {}",
                            event.getRequestId(), event.getNetworkElementIp(),
                     ...
                     */
                    log.debug("HANDOFF : deletion : CHANGED : {}, {}, {}",
                            this.notification.getRequestId(),
                            this.notification.getNetworkElementIp(),
                            this.notification.getNetworkElementType());
                } else {
                    ...
                    if (result == null) {
                        ...
                        log.error(
                                "Deletion of network element identified by {} of type {} failed, unknown reason",
                                this.notification.getNetworkElementIp(),
                                this.notification.getNetworkElementType());
                        final NetworkElementDeletionFailure event = new NetworkElementDeletionFailureBuilder()
                        .setNetworkElementIp(
            ...
            log.error(
                    ...
                    this.notification.getRequestId(), this.notification
                    .getNetworkElementIp(), this.notification
                    .getNetworkElementType(), e.getClass().getName(), e
                    .getMessage());
            try {
                NetworkElementDeletionFailure event = new NetworkElementDeletionFailureBuilder()
                .setRequestId(this.notification.getRequestId())
                ...
                log.error(
                        ...
                        this.notification.getRequestId(), this.notification
                        .getNetworkElementIp(), this.notification
                        .getNetworkElementType(), e.getClass()
