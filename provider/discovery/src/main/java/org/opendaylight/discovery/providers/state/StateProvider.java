 ...
 * @since 2014-07-15
 */
public class StateProvider implements DiscoveryListener,
        DiscoveryIdentificationProviderListener,
        DiscoveryIdentificationListener, DiscoverySynchronizationListener,
        DiscoveryDeletionListener, AutoCloseable {
    private static final Logger log = LoggerFactory
            .getLogger(StateProvider.class);
    ...
    private synchronized void updateState(DiscoveryStateChangeBuilder builder) {
        ...
        final InstanceIdentifierBuilder<DiscoveryState> id = InstanceIdentifier
                .builder(DiscoveryStates.class).child(DiscoveryState.class,
                        new DiscoveryStateKey(builder.getRequestId()));
        ...
        try {
            ...
            log.error(
                    String.format(
                            "Error while attempting to persist status update for request {}, to status {}",
                            builder.getRequestId(), builder.getToState()), e);
        }
        log.debug(
                "STORE : PUT : /discovery-states/discovery-state/{} : {} , {} , {}",
                builder.getRequestId(), builder.getNetworkElementIp(),
                builder.getToState(), builder.getNodeId());
        log.debug("TRANSITION: ReqID {} from {} to {} @ {}",
                builder.getRequestId(), builder.getFromState(),
    ...
    private synchronized void nodeUpdateState(NodeStateChangeBuilder builder) {
        ...
        final InstanceIdentifierBuilder<NodeIdToState> id = InstanceIdentifier
                .builder(NodeIdToStates.class).child(NodeIdToState.class,
                        new NodeIdToStateKey(builder.getNodeId()));
        final ReadWriteTransaction wo = dataBroker.newReadWriteTransaction();
        ...
        log.debug("STORE : PUT : /node-id-to-states/node-id-to-state/{} : {} ",
                builder.getNodeId(), builder.getState());
    }
    ...
    public void onIdentifyNetworkElement(IdentifyNetworkElement notification) {
        ...
        updateState(builder);
        log.debug(
                "EVENT : DiscoveryStateChange : PUBLISH : {}, {}, {}, {}, {}",
                builder.getRequestId(), builder.getNetworkElementIp(),
                builder.getNetworkElementType(), builder.getToState(),
                builder.getNodeId());
     ...
     *
     * @see
     * org.opendaylight.yang.gen.v1.urn.ciena.discovery.identification.rev140714
     * .IdentificationListener# onNetworkElementInProcess
     * (org.opendaylight.yang.
     * gen.v1.urn.opendaylight.discovery.identification.rev140714
     ...
     */
    @Override
    public void onUnableToIdentifyNetworkElement(
            UnableToIdentifyNetworkElement notification) {
        DiscoveryStateChangeBuilder builder = new DiscoveryStateChangeBuilder();
        builder.setRequestId(notification.getRequestId());
        builder.setFromState(State.Identifying);
     ...
     */
    @Override
    public void onNetworkElementSynchronizationFailure(
            NetworkElementSynchronizationFailure notification) {
        DiscoveryStateChangeBuilder builder = new DiscoveryStateChangeBuilder();
        builder.setRequestId(notification.getRequestId());
     ...
     */
    @Override
    public void onNetworkElementSynchronized(
            NetworkElementSynchronized notification) {
        DiscoveryStateChangeBuilder builder = new DiscoveryStateChangeBuilder();
        builder.setRequestId(notification.getRequestId());
        builder.setFromState(State.Synchronizing);
        ...
        build2.setNetworkElementType(notification.getNetworkElementType());
        nodeUpdateState(build2);
        log.debug("EVENT : NodeStateChange : PUBLISH : {}, {}, {}, {}",
                build2.getNodeId(), build2.getState(), builder.getRequestId(),
                builder.getNetworkElementIp());
    ...
    public void onDuplicateIdentity(DuplicateIdentity notification) {
        ...
        builder.setTimestamp(BigInteger.valueOf(new Date().getTime()));
        updateState(builder);
        log.debug("EVENT : DiscoveryStateChange : PUBLISH : {}, {}, {}",
                builder.getRequestId(), builder.getNetworkElementIp(),
                builder.getToState());
        notificationProviderService.publish(builder.build());
    ...
    public void onDeleteNetworkElement(DeleteNetworkElement notification) {
        DiscoveryStateChangeBuilder builder = new DiscoveryStateChangeBuilder();
        builder.setRequestId(notification.getRequestId());
        builder.setFromState(State.Discovered);
        builder.setToState(State.Deleting);
        builder.setNodeId(notification.getNodeId());
        builder.setNetworkElementIp(notification.getNetworkElementIp());
        builder.setTimestamp(BigInteger.valueOf(new Date().getTime()));
        updateState(builder);
    ...
    public void onNetworkElementDeleted(NetworkElementDeleted notification) {
        ...
        builder.setToState(State.Deleted);
        builder.setNodeId(notification.getNodeId());
        builder.setNetworkElementIp(notification.getNetworkElementIp());
        builder.setNetworkElementType(notification.getNetworkElementType());
        builder.setTimestamp(BigInteger.valueOf(new Date().getTime()));
    ...
    public void onNetworkElementDeletionFailure(
        ...
        builder.setRequestId(notification.getRequestId());
        builder.setFromState(State.Deleting);
        builder.setToState(State.DeletionFailed);
        builder.setNodeId(notification.getNodeId());
        builder.setNetworkElementIp(notification.getNetworkElementIp());
        builder.setTimestamp(BigInteger.valueOf(new Date().getTime()));
        updateState(builder);
